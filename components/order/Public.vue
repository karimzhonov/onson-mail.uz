    ...
    <section class="mt-2" id="tracker">
        <div class="w-full relative py-8 md:py-10 px-6 md:px-8 rounded-2xl bg-gradient-to-tr from-gray-100 to-gray-200 ">
            ...
            <div class="mx-auto text-center relative">
                ...
                <h2 v-if="useI18n().locale.value === 'uz'"
                    class="text-gray-800 pt-4  font-bold text-4xl md:text-5xl lg:text-6x">
                    <span
                ...
                <div class="mx-auto max-w-md sm:max-w-xl pt-5">
                    <div class="flex items-center relative gap-x-2">
                        <input v-model="number" type="text" :placeholder="$t('Введите номер отслеживания')"
                            @keydown.enter="fetch_order"
                ...
                <div class="mx-auto max-w-md sm:max-w-4xl mt-3 flex justify-between md:flex-row flex-col-reverse"
                    v-if="order">
                    <div>
                        <div class="flex mb-2">
                            <p class="text-gray-600  text-xl" style="text-wrap: nowrap">{{ $t('Hомер отслеживания') }}: </p>
                            <p class="font-bold text-transparent bg-clip-text bg-gradient-to-br from-primary to-[#8cd66a] ml-2 text-xl"
                                style="text-wrap: nowrap">{{ order.number }}</p>
                        </div>

                        <div class="flex mb-2">
                            <p class="text-gray-600  text-xl flex" style="text-wrap: nowrap">{{ $t('ФИО') }}:</p>
                            <p class="font-bold text-transparent bg-clip-text bg-gradient-to-br from-primary to-[#8cd66a] ml-2 text-xl"
                                lang="ru" style="word-wrap: break-word;hyphens: auto;">{{ order.fio }} </p>
                        </div>

                        <div class="flex mb-2">
                            <p class="text-gray-600  text-xl flex" style="text-wrap: nowrap">{{ $t('Вес товара') }}:</p>
                            <p class="font-bold text-transparent bg-clip-text bg-gradient-to-br from-primary to-[#8cd66a] ml-2 text-xl"
                                lang="ru" style="word-wrap: break-word;hyphens: auto;">{{ order.weight }} {{ $t('кг') }}
                            </p>
                        </div>

                        <div class="flex mb-2">
                            <p class="text-gray-600  text-xl flex" style="text-wrap: nowrap">{{ $t('Оплата за доставку') }}:
                            </p>
                            <p class="font-bold text-transparent bg-clip-text bg-gradient-to-br from-primary to-[#8cd66a] ml-2 text-xl"
                                lang="ru" style="word-wrap: break-word;hyphens: auto;">{{ order.delivery_price }} $ </p>
                        </div>
                        ...
                        <div class="flex flex-col mb-2" v-if="order.products.length > 0">
                            ...
                            <div class="overflow-x-auto w-full">
                                <table class="mt-3 w-full text-sm text-left text-gray-600 ">
                                    ...
                                    <tbody>
                                        <tr v-for="pio in order.products"
                    ...
                    <div class="text-gray-600  md:w-[350px] mb-3">
                        <div class="flex flex-row m-3 py-3" v-for="step in steps"
                            :class="{ 'opacity-60': order.status !== step.step }">
                            <div class="flex flex-col items-center">
                                <div class="w-12 h-12 rounded-[50%] bg-gray-200 text-white flex justify-center items-center"
                                    :class="{ '!bg-green-500': order.status === step.step }">
                                    <component class="w-6 h-6" :is="step.icon"
                                        :class="{ '!text-green-500': order.status !== step.step }" />
                                </div>
                ...
                <div class="mx-auto max-w-md sm:max-w-xl mt-10" v-if="error">
                    <p class="font-bold text-transparent bg-clip-text bg-gradient-to-br from-primary to-[#8cd66a]">
                        {{ $t('order_not_found') }}</p>
                </div>
            </div>
        </div>
